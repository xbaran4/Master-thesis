...
\thesissetup{
    ...
    abstract    = {Jupyter Notebooks are widely used by data scientists, researchers, and educators. As their popularity grew, the demand for a scalable, multi-user solution led to the creation of JupyterHub, which leverages the scalability of Kubernetes. However, for cost-effectiveness, maintenance, or optimizing resource utilization, Pods in the Kubernetes cluster might have to be removed or migrated between the Nodes, which results in application downtime or loss of valuable computational state. This thesis addresses these challenges by exploring the concept of checkpoint/restore for Linux processes, where a process can be frozen, stored on the file system as a set of files, and later recreated in its exact prior state. Additionally, the thesis explores the adaptation of this concept for containers in the Kubernetes cluster. The outcome of this work is a prototype system for Kubernetes clusters, capable of checkpointing and restoring Jupyter Notebooks, which consists of two key components: a standalone application for creating checkpoint container images and a component that integrates the checkpoint/restore functionality into the Z2JH JupyterHub distribution.},
...
The provisioning of virtual machines was accomplished through the user interface of OpenStack in MetaCentrum and, as such, is not part of this thesis. However, a shell script with commands that install and build necessary binaries and then install Kubernetes cluster is provided in \Cref{apendix}. Note that due to the unpredictability of IP addresses and hostnames and the choice of container runtime, the script should be used in a cut-and-paste manner rather than executed as a whole.
